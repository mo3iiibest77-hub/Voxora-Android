...
### Reader library UI
- `ReaderLibrarySection` renders the shelf: a **continue card** for the most recently read book that has a resume point (title, cover, author, progress, chunk position, Continue), then one row per book (cover, title, author, progress bar, "Chunk 12 of 210", state, "Last read …", remove), then an import button. Rows are ordered by recency, and the currently loaded book is marked. Selecting a book loads it at its saved chunk — **it never re-imports**. A book whose copy is gone shows the "Not available" state and is **not clickable**: a row that can only fail is worse than one that plainly cannot be used. A completed book's continue card is still offered; an unavailable book's never is.
