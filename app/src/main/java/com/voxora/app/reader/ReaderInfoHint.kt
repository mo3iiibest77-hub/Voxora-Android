 ...
 *
 * The green explanation sentences explain a section in one or two lines, which is the right amount
 * for the page and the wrong amount for a reader who wants to understand the whole behaviour. An
 * info icon beside the sentence opens a short, section-specific explanation without turning the
 * section into a wall of text — and without changing what the sentence itself says, because the
 * sentence is the summary and the dialog is the detail.
 *
 * ## What it deliberately is not
 *
 * It is not a status or an error: nothing here reports a fault, so the icon uses the explanation
 * role rather than a status colour, and it never appears on a card whose job is to say something is
 * wrong. It is also not a tooltip: a tooltip is unreachable for a touch user and is not announced
 * reliably, whereas a dialog is focusable, dismissible and readable by a screen reader.
 *
...
internal fun HelpIconButton(
    ...
    IconButton(
        ...
        Icon(
            ...
            tint = VoxoraColors.explanation,
            modifier = Modifier.size(HELP_ICON_DP.dp),
        )
    }
    if (showing) {
        AlertDialog(
...
/** The glyph stays small; the touch target stays Material's, which is what accessibility needs. */
private const val HELP_ICON_DP = 18
